...
class ApiService(val cxt: Context) : IApiService {
    ...
    override suspend fun checkSignIn(account: String, deviceId: String, fcmToken: String): Response<SignInInfo> {
        delay(1500)
         ...
         * 測試參數不同時會執行失敗
         * */
        return if (account == "north27" && deviceId == "deviceId001" && fcmToken == user.fcmToken)
            Response.success<SignInInfo>(
                SignInInfo(
            ...
            Response.success<SignInInfo>(
                SignInInfo(signInState = SignInState.FAILURE)
            )
    ...
    override suspend fun postSignIn(account: String, password: String, deviceId: String, fcmToken: String): Response<SignInInfo> {
        delay(1500)
        return if (account == "north27" && password == "north27")
            Response.success<SignInInfo>(
                SignInInfo(
                    signInState = SignInState.SUCCESS,
                    isFirst = true,
