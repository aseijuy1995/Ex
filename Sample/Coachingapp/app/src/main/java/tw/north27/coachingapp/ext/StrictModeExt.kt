...
fun startStrictMode() {
    StrictMode.setThreadPolicy(
        StrictMode.ThreadPolicy.Builder().apply {
            ...
            detectResourceMismatches()//啟用檢測已定義資源類型和getter調用之間的不匹配的功能。
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) detectUnbufferedIo()//檢測未緩衝的輸入/輸出操作。
