...
class PersonalCommentFragment : BaseFragment<FragmentPersonalCommentBinding>(R.layout.fragment_personal_comment) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.srlView.setOnRefreshListener {
            ...
            val unit = if (binding.itemDrawerLayoutComment.spUnit.selectedItem is Unit) (binding.itemDrawerLayoutComment.spUnit.selectedItem as Unit) else null
            getCommentList(score = score, education = education, grade = grade, subject = subject, unit = unit, index = 0, num = 10)
        }
    ...
    private fun setUiData(scoreList: List<Double>?, educationList: List<Education>, gradeList: List<Grade>, subjectList: List<Subject>, unitList: List<Unit>) {
        ...
        binding.itemDrawerLayoutComment.spSubject.setSelection(0)
        unitAdapter.submitData(unitList.toMutableList().apply { add(0, publicVM.defaultUnit) })
        binding.itemDrawerLayoutComment.spUnit.setSelection(0)
    }

    private fun getCommentList(score: Double? = null, education: Education? = null, grade: Grade? = null, subject: Subject? = null, unit: Unit? = null, index: Int, num: Int) {
        logD(
            "score = $score\n" +
