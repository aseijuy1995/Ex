...
class MediaModule(val context: Context) : IMediaModule {

    private val albumMap = mutableMapOf<String, MediaAlbum>()


    private val mediaAlbumItemRelay = PublishRelay.create<List<MediaAlbum>>()

    private val isEmpty: Boolean = albumMap.keys.isEmpty()
...
//        }
//        cursor?.close()
//        mediaAlbumItemRelay.accept(albumMap.values.toList())
    }

    private fun addMediaAlbumItem(albumName: String, folder: String, covertPath: String) {
        albumMap[albumName] ?: MediaAlbum(name = albumName, folder = folder, covertPath = covertPath).also {
            albumMap[albumName] = it
        }
    }
...
//    override fun getAlbumItems(setting: MediaSetting): PublishRelay<List<MediaAlbumItem>> {
//        if (isEmpty)
...
//    }
//
//    override fun getAlbumItemFromAlbumName(albumName: String, setting: MediaSetting): Observable<MediaAlbumItem?> {
//        if (isEmpty)
//            fetchMedia(setting = setting)
//                .subscribeOn(Schedulers.io())
//                .subscribe()
...
//    }
//
//    override fun getAlbumItemSync(albumName: String, setting: MediaSetting): MediaAlbumItem? {
//        if (isEmpty)
//            fetchMedia(setting = setting)
//
